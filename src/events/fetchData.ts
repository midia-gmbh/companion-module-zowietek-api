...
export async function fetchData(instance: ZowietekInstance, options: { general?: boolean; ndi?: boolean; storage?: boolean }): Promise<void> {
    if (!instance.globalSettings.enableComs || !instance.api || !instance.connected) {
        return;
    }

    // Fetch allgemeine Daten (alle 1 Sekunde)
    if (options.general) {
        ...
        if (getInputSignal.status === ZowieStatus.Successful || getInputSignal.status === ZowieStatus.ModificationSuccessful) {
            ...
            inputVariableConfigs.forEach(config => {
                ...
                value = config.type === 'boolean' ? value === 1 : value?.toString();
    ...
    // Fetch NDI Decode Status (alle 5 Sekunden)
    if (options.ndi) {
    ...
    // Fetch Storage Data (alle 5 Sekunden)
    if (options.storage) {
        const getRecordingStatus = await instance.api.getRecordingTaskList();
